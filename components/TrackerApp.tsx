...
export function TrackerApp({ onReturnHome }: { onReturnHome?: () => void } = {}) {
  ...
  const planningSettings = useMemo(() => getPlanningSettings(backup), [backup]);
  ...
  function exportBackup() {
    const exportable = {
      ...
      settings: {
        ...backup.settings,
        planning: getPlanningSettings(backup),
        displayCurrency: getDisplayCurrencySettings(backup),
        taxProfile: getTaxProfile(backup)
      }
    };
...
function PlanningView({ backup, settings, scenario, rebalancing, goalRebalancing, cashFlowPlan, incomeProjection, attribution, currency, usdSecondary }: { backup: PortfolioBackup; settings: PlanningSettings; scenario: ScenarioPlan; rebalancing: RebalanceRow[]; goalRebalancing: GoalRebalancePlan[]; cashFlowPlan: CashFlowPlan; incomeProjection: IncomeProjection; attribution: PerformanceAttribution; currency: string; usdSecondary: (value: number | undefined) => string | undefined }) {
  ...
  return (
    <section className="grid planning-workspace">
      <div className="card wide-card planning-hero-card">
        <div className="section-head">
          <div><h2>Planning Lab</h2><p>Decision-support analytics built from the real canonical portfolio. Scenario, rebalance, and drawdown assumptions are saved in Settings and never mutate actual values, transactions, tax lots, or XIRR.</p></div>
        </div>
        <div className="holding-command-strip">
          <MiniInsight label="Current value" value={formatMoney(scenario.currentValue, currency)} secondary={usdSecondary(scenario.currentValue)} detail="real portfolio total" />
          <MiniInsight label="Stress value" value={formatMoney(scenario.stressedValue, currency)} secondary={usdSecondary(scenario.stressedValue)} detail={formatMoney(scenario.stressDelta, currency) + " scenario delta"} />
          <MiniInsight label="One-year scenario" value={formatMoney(scenario.oneYearProjectedValue, currency)} secondary={usdSecondary(scenario.oneYearProjectedValue)} detail="using configured returns" />
          <MiniInsight label="Largest drift" value={largestRebalance ? largestRebalance.category : "-"} detail={largestRebalance ? formatMoney(Math.abs(largestRebalance.driftValue), currency) + " " + largestRebalance.action : "target matched"} />
        </div>
      </div>
...
function AttributionBridge({ attribution, currency }: { attribution: PerformanceAttribution; currency: string }) {
  return (
    <div className="attribution-bridge">
      <div className="holding-command-strip compact-command-strip">
        <MiniInsight label="External cash in" value={formatMoney(attribution.externalCashIn, currency)} detail="capital entering portfolio" />
        <MiniInsight label="External cash out" value={formatMoney(attribution.externalCashOut, currency)} detail="capital returned/withdrawn" />
        <MiniInsight label="Income" value={formatMoney(attribution.income, currency)} detail="dividend/interest rows" />
        <MiniInsight label="Market gain" value={formatMoney(attribution.marketGain, currency)} detail="current value less cost basis" />
      </div>
      <div className="metric-bar-list attribution-list">{attribution.rows.map((row, index) => {
...
function SnapshotsView(props: {
  ...
  const [compareFromId, setCompareFromId] = useState(() => props.backup.snapshots[0]?.id ?? "");
  const [compareToId, setCompareToId] = useState(() => props.backup.snapshots.at(-1)?.id ?? "");
  const snapshotComparison = useMemo(() => {
    const from = props.backup.snapshots.find((snapshot) => snapshot.id === (compareFromId || props.backup.snapshots[0]?.id));
    const to = props.backup.snapshots.find((snapshot) => snapshot.id === (compareToId || props.backup.snapshots.at(-1)?.id));
    return from && to && from.id !== to.id ? compareSnapshots(from, to) : null;
  }, [compareFromId, compareToId, props.backup.snapshots]);
...
function SnapshotComparisonPanel({ snapshots, comparison, compareFromId, compareToId, setCompareFromId, setCompareToId, currency }: { snapshots: PortfolioBackup["snapshots"]; comparison: SnapshotComparison | null; compareFromId: string; compareToId: string; setCompareFromId: (value: string) => void; setCompareToId: (value: string) => void; currency: string }) {
  const deltaRows = comparison ? categoryOrder.map((category) => ({ name: category, value: Math.abs(comparison.categoryDeltas[category]), tag: comparison.categoryDeltas[category] >= 0 ? "up" : "down", category })).filter((row) => row.value > 0) : [];
  return (
    <div className="card wide-card snapshot-compare-card">
      <div className="section-head"><div><h2>Snapshot Comparison</h2><p>Compare two frozen JSON snapshots without fetching market data. Deltas show what changed between saved states.</p></div></div>
      <div className="snapshot-compare-controls">
        <label><span>From</span><select value={compareFromId} onChange={(event) => setCompareFromId(event.target.value)}>{snapshots.map((snapshot) => <option value={snapshot.id} key={snapshot.id}>{snapshot.name} · {snapshot.asOfDate}</option>)}</select></label>
        <label><span>To</span><select value={compareToId} onChange={(event) => setCompareToId(event.target.value)}>{snapshots.map((snapshot) => <option value={snapshot.id} key={snapshot.id}>{snapshot.name} · {snapshot.asOfDate}</option>)}</select></label>
      </div>
      {!comparison ? <p className="message">Choose two different snapshots to compare.</p> : <><div className="holding-command-strip"><MiniInsight label="Net worth delta" value={formatMoney(comparison.netWorthDelta, currency)} detail={comparison.fromName + " → " + comparison.toName} /><MiniInsight label="Invested delta" value={formatMoney(comparison.investedDelta, currency)} detail="frozen cost basis change" /><MiniInsight label="Profit delta" value={formatMoney(comparison.profitDelta, currency)} detail="frozen P/L change" /><MiniInsight label="Goal readiness delta" value={formatMoney(comparison.goalMappedDelta - comparison.goalRequiredTodayDelta, currency)} detail="mapped minus need movement" /></div><ChartCard title="Asset Class Delta"><RankingBar data={deltaRows} formatValue={(value) => formatMoney(value, currency)} emptyMessage="No asset-class movement between selected snapshots." /></ChartCard></>}
    </div>
  );
}
...
function GoalCard({ progress, currency, usdSecondary, selected, setSelectedGoalId, updateGoalRecord, deleteGoalRecord }: { progress: GoalProgress; currency: string; usdSecondary: (value: number | undefined) => string | undefined; selected: boolean; setSelectedGoalId: (value: string) => void; updateGoalRecord: (goalId: string, patch: Partial<Goal>) => void; deleteGoalRecord: (goalId: string) => void }) {
  ...
  return (
    <div className={"goal-card" + (selected ? " selected" : "")} onClick={() => setSelectedGoalId(goal.id)}>
      ...
      <p className="message compact-note">Goal assumptions, returns, accumulation targets, pre-goal glidepath, consumption targets, and drawdown glidepath are edited in Settings. This page focuses on readiness and asset mapping.</p>
    ...
    <section className="grid imports-workspace">
      ...
      <div className="grid two">
        <div className="card">
          ...
          {props.nativeDetection && <ImportDetectionPanel detection={props.nativeDetection} fileCount={props.nativeFileCount} />}
          ...
          <NativeStagedPreviewPanel rows={stagedPreview} currency={props.backup.baseCurrency} />
        </div>
        <SupportedImportsMatrix />
      </div>
      <div className="grid two">
...
function ImportDetectionPanel({ detection, fileCount }: { detection: ImportDetection; fileCount: number }) {
  ...
  return (
    <div className={"detection import-detection-panel " + (implemented && !isUnknown ? "supported" : "needs-review")}>
      ...
      <div><span>Status</span><strong>{isUnknown ? "unsupported" : importSupportLabel(detection.status)}</strong></div>
      <div><span>Type</span><strong>{detection.nativeInputType}</strong></div>
      <div><span>Confidence</span><strong>{detection.confidence}</strong></div>
      <p>{isUnknown ? "This file does not match any known native importer or manual CSV template. No parser will run until the file matches a supported format." : detection.reason}</p>
    </div>
  );
}
...
function SupportedImportsMatrix() {
  const implemented = providerImportSpecs.filter((spec) => spec.status === "implemented" || spec.status === "parser_implemented");
  const waiting = providerImportSpecs.filter((spec) => spec.status !== "implemented" && spec.status !== "parser_implemented");
  return (
    <div className="card supported-imports-card">
      <div className="section-head compact-section-head"><div><h2>Supported Imports</h2><p>Known file formats only. Unknown files are rejected instead of guessed so portfolio data stays trustworthy.</p></div></div>
      <div className="support-matrix-summary">
        <MiniInsight label="Ready now" value={String(implemented.length)} detail="verified import paths" />
        <MiniInsight label="Needs sample" value={String(waiting.length)} detail="detected or research only" />
      </div>
      <div className="support-matrix-list">
        {providerImportSpecs.map((spec) => <div className="support-matrix-row" key={spec.id}>
          <div><strong>{spec.label}</strong><span>{spec.assetTypes.join(", ")}</span></div>
          <em>{spec.nativeInputTypes.join(" / ")}</em>
          <strong className={"status-pill " + spec.status}>{importSupportLabel(spec.status)}</strong>
          <small>{spec.implementation}</small>
        </div>)}
      </div>
      <DrilldownPanel title="How unsupported files are handled" summary="No guessing; add a fixture-backed parser.">
        <p className="message">If a file does not match these formats, the tool should reject it with guidance. New brokers or banks should be added as importer adapters only after real anonymized sample files are available and covered by tests.</p>
      </DrilldownPanel>
    </div>
  );
}

function importSupportLabel(status: string): string {
  if (status === "implemented") return "ready";
  if (status === "parser_implemented") return "parser ready";
  if (status === "detected_only") return "needs parser";
  return "needs sample";
}
...
function TaxView({ report, currency, financialYears, selectedFinancialYear, setSelectedFinancialYear }: { report: ReturnType<typeof calculatePortfolioTaxReport>; currency: string; financialYears: string[]; selectedFinancialYear: string; setSelectedFinancialYear: (value: string) => void }) {
  ...
  function exportTaxSummary() {
    ...
    const link = document.createElement("a");
    link.href = URL.createObjectURL(blob);
    link.download = "portfolio-tax-summary-" + selectedFinancialYear + ".json";
    link.click();
    URL.revokeObjectURL(link.href);
  }
  return (
    <section className="grid tax-workspace">
      <div className="card wide-card">
        <div className="section-head">
          ...
          <div className="tax-head-actions"><label className="tax-year-picker"><span>Financial year</span><select value={selectedFinancialYear} onChange={(event) => setSelectedFinancialYear(event.target.value)}>{financialYears.map((year) => <option value={year} key={year}>FY {year}</option>)}</select></label><button onClick={exportTaxSummary}><Download size={15} /> Export FY Summary</button></div>
        </div>
        <div className="holding-command-strip">
  ...
  return (
    <section className="grid data-audit-workspace">
      ...
      <div className="analytics-grid">
        ...
        <DrilldownPanel title="Market Data Gaps" summary={report.marketDataGaps.length === 0 ? "No current gaps" : report.marketDataGaps.length + " review item(s)"}>{report.marketDataGaps.length === 0 ? <p className="message">No current FX/price/NAV gaps detected.</p> : <div className="table-wrap"><table><thead><tr><th>Kind</th><th>Label</th><th>Date</th><th>Severity</th></tr></thead><tbody>{report.marketDataGaps.slice(0, 40).map((gap) => <tr key={gap.kind + gap.label + gap.date}><td>{gap.kind}</td><td>{gap.label}</td><td>{gap.date ?? "-"}</td><td>{gap.severity}</td></tr>)}</tbody></table></div>}</DrilldownPanel>
        <DrilldownPanel title="Market Data Health" summary={report.marketDataHealth.length === 0 ? "No holdings need market data" : report.marketDataHealth.length + " coverage row(s)"}><p className="chart-note compact-note">Current valuation coverage by holding and FX pair. Covered rows show the stored source/date; missing critical rows block trustworthy INR totals.</p>{report.marketDataHealth.length === 0 ? <p className="message">No holdings need market data yet.</p> : <div className="table-wrap"><table><thead><tr><th>Status</th><th>Kind</th><th>Label</th><th>As of</th><th>Source</th><th>Detail</th></tr></thead><tbody>{report.marketDataHealth.slice(0, 80).map((row) => <tr key={row.kind + row.label + row.status + row.asOfDate}><td><span className={"status-pill " + (row.status === "covered" ? "implemented" : row.severity === "critical" ? "planned" : "partial")}>{row.status}</span></td><td>{row.kind}</td><td>{row.label}</td><td>{row.asOfDate ?? "-"}</td><td>{row.source ?? "-"}</td><td>{row.detail}</td></tr>)}</tbody></table></div>}</DrilldownPanel>
        <DrilldownPanel title="Import Quality" summary={report.imports.length === 0 ? "No committed imports" : report.imports.length + " import(s)"}>{report.imports.length === 0 ? <p className="message">No committed imports yet.</p> : <div className="table-wrap"><table><thead><tr><th>Import</th><th>Provider</th><th>Records</th><th>Confidence</th></tr></thead><tbody>{report.imports.map((run) => <tr key={run.id}><td>{run.label}</td><td>{run.provider}</td><td>{run.records}</td><td>{run.confidence}</td></tr>)}</tbody></table></div>}</DrilldownPanel>
      </div>
      <DrilldownPanel title="Source Ledger Totals" summary={report.sourceTotals.length === 0 ? "No source records" : report.sourceTotals.length + " source(s)"}>{report.sourceTotals.length === 0 ? <p className="message">No source records yet.</p> : <div className="table-wrap"><table><thead><tr><th>Source</th><th>Holdings</th><th>Transactions</th><th>Current Value</th></tr></thead><tbody>{report.sourceTotals.map((row) => <tr key={row.source}><td>{row.source}</td><td>{row.holdings}</td><td>{row.transactions}</td><td>{formatMoney(row.value, currency)}</td></tr>)}</tbody></table></div>}</DrilldownPanel>
    </section>
  );
}
...
function SettingsView({ profile, updateTaxProfile, displaySettings, updateDisplaySettings, planningSettings, updatePlanningSettings, goals, goalExpenses, updateGoalSettings, currency }: { profile: TaxProfile; updateTaxProfile: (patch: Partial<TaxProfile>) => void; displaySettings: DisplayCurrencySettings; updateDisplaySettings: (patch: Partial<DisplayCurrencySettings>) => void; planningSettings: PlanningSettings; updatePlanningSettings: (patch: PlanningSettingsPatch) => void; goals: Goal[]; goalExpenses: GoalExpense[]; updateGoalSettings: (patches: Array<{ goalId: string; patch: Partial<Goal> }>) => void; currency: string }) {
  ...
  return (
    <section className="grid settings-workspace">
      <div className="card wide-card settings-command-card">
        <div className="section-head"><div><h2>Settings</h2><p>Central place for assumptions and preferences. These values are stored in the canonical JSON backup and restored across browsers.</p></div><div className="draft-actions compact-draft-actions"><button className="primary" disabled={!settingsDirty} onClick={applySettings}>Done editing settings</button><button disabled={!settingsDirty} onClick={resetSettings}>Reset draft</button></div></div>
        <div className="holding-command-strip compact-command-strip">
          <MiniInsight label="Base currency" value={currency} detail="all calculations stay in INR base" />
          <MiniInsight label="Tax slab" value={(draftProfile.slabRate ?? 0) + "%"} detail="portfolio tax estimate" />
          <MiniInsight label="USD display" value={draftDisplaySettings.showUsdEquivalent ? "On" : "Off"} detail="display-only secondary values" />
          <MiniInsight label="Goals" value={String(goals.length)} detail="goal assumptions below" />
        </div>
        <p className="message compact-note">Most users only need Imports, Holdings, Analytics, and Backup. Use Settings when you want to tune tax estimates, display preferences, planning scenarios, goal assumptions, glidepaths, and contribution planning.</p>
      </div>

      <div className="settings-section-list">
        <details className="settings-section-card">
          <summary><span>Tax estimate inputs</span><small>Resident Indian portfolio-tax assumptions. This is not a salary or ITR engine.</small></summary>
          ...
          <p className="message compact-note">The Tax section estimates portfolio tax only. Salary, deductions, Form 16, full ITR schedules, and ESPP payroll/perquisite treatment stay outside this tracker unless explicitly added later.</p>
        </details>

        <details className="settings-section-card" open>
          <summary><span>Display preferences</span><small>Cosmetic options that do not change the calculation base.</small></summary>
        ...
        <details className="settings-section-card">
          <summary><span>Portfolio planning assumptions</span><small>Scenario, target allocation, contribution planning, and advisory rebalancing inputs.</small></summary>
          <PlanningSettingsDraftEditor settings={planningSettings} goals={goals} onApply={updatePlanningSettings} mode="all" />
        </details>

        <details className="settings-section-card" open>
          <summary><span>Goal assumptions and glidepaths</span><small>Goal-level expense source, return assumptions, accumulation glidepath, drawdown, and target allocations.</small></summary>
  ...
  return (
    <div className="goal-settings-card">
      ...
      <details className="goal-settings-subsection">
        <summary><span>Return assumptions</span><small>Annual category returns used for goal projections, readiness, and planning math.</small></summary>
        <div className="settings-grid compact-settings-grid goal-settings-fields">
      ...
      <details className="goal-settings-subsection">
        <summary><span>Drawdown and consumption glidepath</span><small>Post-goal withdrawal model and optional allocation glide after the goal starts.</small></summary>
        <div className="settings-grid compact-settings-grid goal-settings-fields">
          ...
          <label><FieldLabel label="Shift every N years" help="How often the consumption allocation glides from one asset class to another." /><input type="number" min="1" max="50" value={goal.consumptionGlideIntervalYears ?? 5} onChange={(event) => updateDraftGoal(goal.id, { consumptionGlideIntervalYears: Number(event.target.value) })} /></label>
          <label><FieldLabel label="Shift amount %" help="Percentage points moved at each glide interval. Example: 5 moves equity from 50% to 45%." /><input type="number" min="0" max="100" step="1" value={goal.consumptionGlideShiftPercent ?? 0} onChange={(event) => updateDraftGoal(goal.id, { consumptionGlideShiftPercent: Number(event.target.value) })} /></label>
...
function ImportPreviewPanel({ preview, currency }: { preview: ManualImportPreview | null; currency: string }) {
  ...
  return (
    <div className="import-preview-panel">
      ...
      <DrilldownPanel title="Preview row details" summary={preview.rows.length + " action row(s)"} defaultOpen><div className="table-wrap"><table><thead><tr><th>Action</th><th>Kind</th><th>Record</th><th>Detail</th></tr></thead><tbody>{preview.rows.slice(0, 60).map((row, index) => <tr key={row.kind + row.label + row.action + index}><td>{row.action}</td><td>{row.kind}</td><td>{row.label}</td><td>{row.detail}</td></tr>)}</tbody></table></div></DrilldownPanel>
    </div>
  );
}
...
function nativeStagedPreviewRows(backup: PortfolioBackup, replaceImportId: string | undefined, ...groups: NativePreviewImport[][]): NativeStagedPreviewRow[] {
  return groups.flatMap((imports) => imports.length === 0 ? [] : imports.map((item) => {
    ...
    return {
      ...
      label: item.importRun?.label ?? item.importRun?.fileName ?? item.importRun?.provider ?? "Staged import",
...
function NativeStagedPreviewPanel({ rows, currency }: { rows: NativeStagedPreviewRow[]; currency: string }) {
  ...
  return (
    <div className="native-preview-panel">
      ...
      <p className="message">Parsed native files are staged here before commit. Review counts and warnings, then commit to write canonical records.</p>
      <div className="holding-command-strip compact-command-strip">
        <MiniInsight label="Staged holdings" value={String(totals.balances)} detail="balances ready to commit" />
...
function HoldingDetailDrawer({ backup, holding, returns, taxReport, goalProgress, currency, close, usdSecondary }: { backup: PortfolioBackup; holding: ReturnType<typeof calculatePortfolioInsights>["holdings"][number]; returns?: HoldingReturn; taxReport: ReturnType<typeof calculatePortfolioTaxReport>; goalProgress: GoalProgress[]; currency: string; close: () => void; usdSecondary: (value: number | undefined) => string | undefined }) {
  const balance = backup.manualBalances.find((item) => item.id === holding.id);
  const transactions = balance ? backup.transactions.filter((tx) => tx.accountId === balance.accountId && (balance.instrumentId ? tx.instrumentId === balance.instrumentId : !tx.instrumentId)).sort((a, b) => b.date.localeCompare(a.date)).slice(0, 80) : [];
  const goalRows = goalProgress.flatMap((goal) => goal.mappedHoldings.filter((mapped) => mapped.balance.id === holding.id).map((mapped) => ({ goal: goal.goal.name, percent: mapped.mappedPercent, value: mapped.value, projected: mapped.projectedValue })));
  const realizedTaxRows = taxReport.realized.byAssetBucket.filter((row) => row.assetName === holding.label || displayHoldingName(row.assetName) === displayHoldingName(holding.label));
  const unrealizedTaxRows = taxReport.unrealized.byAssetBucket.filter((row) => row.assetName === holding.label || displayHoldingName(row.assetName) === displayHoldingName(holding.label));
  return (
    <div className="holding-detail-drawer card wide-card" role="dialog" aria-label="Holding detail">
      <div className="section-head"><div><h2>{displayHoldingName(holding.label)}</h2><p>{holding.assetKind} · {holding.region} · {holding.provider} · detailed ledger and planning context</p></div><button onClick={close}>Hide details</button></div>
      <div className="holding-command-strip compact-command-strip">
        <MiniInsight label="Current value" value={holding.valueInBase === undefined ? "FX needed" : formatMoney(holding.valueInBase, currency)} secondary={usdSecondary(holding.valueInBase)} detail={holding.quantity === undefined ? "no quantity" : formatNumber(holding.quantity) + " units/shares"} />
        <MiniInsight label="Cost basis" value={returns?.costBasisKnown ? formatMoney(returns.netInvested, currency) : "-"} detail={returns?.costBasisKnown ? "remaining basis" : "not available"} />
        <MiniInsight label="Profit / Loss" value={returns?.profit === undefined ? "-" : formatMoney(returns.profit, currency)} detail={returns?.returnPercent === undefined ? "return unavailable" : returns.returnPercent.toFixed(1) + "% simple"} />
        <MiniInsight label="XIRR" value={returns?.xirr === undefined || returns?.xirr === null ? "-" : returns.xirr.toFixed(2) + "%"} detail={returns?.hasCashFlows ? "cash-flow return" : "needs transactions"} />
      </div>
      <div className="detail-drawer-grid">
        <div className="detail-pane"><h3>Goal Mappings</h3>{goalRows.length === 0 ? <p className="message">Not mapped to a goal.</p> : <div className="table-wrap"><table><thead><tr><th>Goal</th><th>Mapped %</th><th>Current</th><th>Projected</th></tr></thead><tbody>{goalRows.map((row) => <tr key={row.goal}><td>{row.goal}</td><td>{row.percent.toFixed(1)}%</td><td>{formatMoney(row.value, currency)}</td><td>{formatMoney(row.projected, currency)}</td></tr>)}</tbody></table></div>}</div>
        <div className="detail-pane"><h3>Tax Lots</h3>{realizedTaxRows.length === 0 && unrealizedTaxRows.length === 0 ? <p className="message">No tax rows for selected FY.</p> : <div className="table-wrap"><table><thead><tr><th>Mode</th><th>Bucket</th><th>Qty</th><th>Gain/Loss</th><th>Tax</th></tr></thead><tbody>{realizedTaxRows.map((row) => <tr key={row.assetName + row.bucket + "realized"}><td>Realized</td><td>{taxBucketLabel(row.bucket)}</td><td>{formatNumber(row.quantity)}</td><td>{formatMoney(row.gain, currency)}</td><td>{formatMoney(row.allocatedTaxAfterSetoff, currency)}</td></tr>)}{unrealizedTaxRows.map((row) => <tr key={row.assetName + row.bucket + "unrealized"}><td>Unrealized</td><td>{taxBucketLabel(row.bucket)}</td><td>{formatNumber(row.quantity)}</td><td>{formatMoney(row.gain, currency)}</td><td>{formatMoney(row.potentialTaxBeforeSetoff, currency)}</td></tr>)}</tbody></table></div>}</div>
      </div>
      <div className="detail-pane"><h3>Recent Transactions</h3>{transactions.length === 0 ? <p className="message">No matching transactions for this holding.</p> : <div className="table-wrap"><table><thead><tr><th>Date</th><th>Type</th><th>Amount</th><th>Qty</th><th>Fees/tax</th><th>Source</th></tr></thead><tbody>{transactions.map((tx) => <tr key={tx.id}><td>{tx.date}</td><td>{tx.type}</td><td>{formatMoney(tx.amount, tx.currency)}</td><td>{tx.quantity === undefined ? "-" : formatNumber(tx.quantity)}</td><td>{tx.fees || tx.taxes ? formatMoney((tx.fees ?? 0) + (tx.taxes ?? 0), tx.currency) : "-"}</td><td>{tx.source.provider ?? tx.source.type}</td></tr>)}</tbody></table></div>}</div>
    </div>
  );
}
