...
- PF/NPS yearly imports must retain all transactions from every imported year while keeping current balances from the latest statement date only. PF parsing must prefer the detailed employee/employer/pension table over any combined summary table and must use detailed monthly contribution rows when the statement provides them. Do not depend on private filenames; parse by verified statement format.
- Keep PF categorized as Debt and NPS categorized from explicit scheme type: E as Equity, C/G as Debt, A as Others.
- Private validation tests must include an independent parser-to-report math audit for remaining cost basis, P/L, simple return, and XIRR when real private fixtures are available.
