...
- Tax UI changes must preserve an auditable trace from displayed gain/tax back to sale transaction, FIFO source lot, optional transaction FMV/tax price, FX conversion, tax bucket reason, and formula; do not expose tax totals without a drilldown path.
...
The app is INR-first. Optional USD equivalents are display-only secondary labels from the latest stored real USD/INR snapshot and must not change INR base values, tax, XIRR, imports, exports, refresh math, or history charts. Current foreign-currency holdings require latest FX. Foreign-currency transaction analytics require FX on or before each transaction date. Do not mix local currency values into INR totals without an FX snapshot, and do not fabricate missing FX.
