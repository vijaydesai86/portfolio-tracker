...
- `monthly-all-assets-template.csv`: single monthly ledger for new investments across mutual funds, Indian stocks, US stocks/Fidelity, PPF, SSY, EPF, NPS, FD, ESPP, cash, gold, and other custom assets. Market-priced rows use quantity and price; balance-style rows use amount. Optional `FMV` is tax-only and is ignored by portfolio cost/valuation math.
- `goal-expenses-retirement.csv`, `goal-expenses-bhoomi.csv`, `goal-expenses-chinnu.csv`: importable goal-expense ledgers. These are separate from portfolio transactions. Rows are detail-first: category totals are computed from line items, `quantity * unit_amount` is supported, and `yearly` rows are divided by 12 for monthly spread items such as Adhoc. `active_scenario=yes` marks the scenario that feeds the goal corpus math.
