...
- Separate Settings workspace for user assumptions such as tax regime, slab, surcharge, cess, and the display-only USD-equivalent toggle, stored inside the canonical JSON backup. USD equivalents are secondary cosmetic values from the latest stored real USD/INR snapshot and do not change INR math, tax, XIRR, imports, exports, or history.
...
- Import pipeline with validation, deduplication, manual CSV impact preview before commit, named import history, import deletion, review-oriented error reporting, commit history, and manual-edit preservation.
