...
- Separate Data and Reconciliation workspace that audits import runs, source-ledger totals, validation checks, market-data gaps, parser warnings, and source value mix before analytics are trusted. Responsive rendering is treated as part of correctness: charts, ranking rows, goal mappings, and wide audit tables must remain usable across desktop, laptop, tablet, and mobile browser widths. Dense chart cards follow the Holdings workspace pattern by stacking before labels collide, and wide finance/audit tables keep numeric cells on one line inside their own scroll area instead of causing page-level overflow or broken digit stacks.
