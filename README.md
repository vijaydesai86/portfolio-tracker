...
- Implemented simple manual CSV import for three use cases: transaction CSVs for market-priced assets such as mutual funds, Indian stocks, US/Fidelity-style stocks, and gold units; balance CSVs for current manually valued assets; and a compact dated balance-ledger CSV for PPF, SSY, ESPP, and cash-style rows where `invest` adds invested/current value and `interest` adds current value only. Balance CSVs accept optional `invested_amount`, `invested_currency`, and `invested_as_of_date`; without those fields profit/return stay unavailable instead of being fabricated. Transaction CSVs derive open holdings from net quantity and use market refresh for current prices/FX when real data is available.
