...
- Separate Planning workspace for advisory analytics that must not mask factual portfolio math: scenario projection, stress impact, target-allocation drift, goal-level suggested rebalance actions, portfolio income projection, performance attribution, snapshot comparison, and goal corpus longevity/drawdown. Drawdown spend growth, corpus consumption years, and withdrawal timing are goal-level assumptions because each goal can behave differently; global Settings remain defaults for scenario/rebalance planning. These panels use existing canonical holdings, goals, market snapshots, and saved snapshots, but they remain planning overlays; they do not mutate imports, actual current value, tax, XIRR, cost basis, or frozen snapshot values.
